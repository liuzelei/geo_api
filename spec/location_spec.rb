...
describe " test coords translate" do
  ...
  it "should coord to gcj" do
    ...
    data_back = GeoApi::LocationService.instance.coord_to_gcj(coords.join(';'), "1")

    data_back.each_with_index do |result, index|
      if index % 2 == 0
        expect(result["lng"].to_s).to eq("-103.48172693")
        expect(result["lat"].to_s).to eq("33.25230179")
      else
        expect(result["lng"].to_s).to eq("-103.4834112")
        expect(result["lat"].to_s).to eq("33.25206831")
      end
    end
